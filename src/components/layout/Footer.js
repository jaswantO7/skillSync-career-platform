  ...
  return (
    <footer className="bg-surface-900 dark:bg-surface-950 text-white border-t border-surface-800">
      <div className="max-w-7xl mx-auto px-2 py-16">
        {/* Top: Brand + Newsletter side by side */}
        <div className="flex flex-col md:flex-row justify-between items-start gap-10 mb-14">
              ...
              {socialLinks.map((item) => {
                ...
                return (
                  <Link
                    key={item.name}
                    href={item.href}
                    className="w-9 h-9 rounded-lg bg-surface-800 hover:bg-stitch-primary flex items-center justify-center text-surface-400 hover:text-white transition-all"
                  >
                    <Icon size={16} />
                  </Link>
                )
          ...
          <div className="w-full md:w-80">
            ...
            <p className="text-surface-400 text-sm mb-4">Get the latest updates and career growth tips.</p>
            <div className="flex flex-col sm:flex-row gap-2">
              <input
                type="email"
                placeholder="Enter your email"
                className="flex-1 px-4 py-2.5 bg-surface-800 border border-surface-700 rounded-xl text-white placeholder-surface-500 focus:outline-none focus:ring-2 focus:ring-stitch-primary/50 focus:border-stitch-primary text-sm transition-all"
              />
              <button className="px-5 py-2.5 bg-stitch-primary hover:bg-stitch-primary/90 text-white font-medium rounded-xl text-sm transition-all whitespace-nowrap">
                Subscribe
              </button>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 lg:gap-12 mb-14">
          {Object.entries(sections).map(([title, links]) => (
            <div key={title}>
              <h3 className="font-semibold text-sm text-surface-300 mb-4 uppercase tracking-wider">{title}</h3>
        ...
        <div className="border-t border-surface-800 pt-8 flex flex-col md:flex-row justify-between items-center">
          <p className="text-surface-500 text-sm">&copy; 2024 SkillSync. All rights reserved.</p>
          <p className="text-surface-500 text-sm mt-2 md:mt-0">Built for career growth</p>
        </div>
      </div>
    </footer>
  )
